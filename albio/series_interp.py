...
def interpPoly(X, t, y, x0=[None], p0=[None]):
    ...
    if not any(x0):
        x0 = [1.50208343e+01, -5.06158347e+00, 3.59972679e-01, 2.50569233e-01, -2.91416175e-02, 1.09982221e-03,
              -1.37937148e-05]
        p0 = [0]
...
def interpFun(X, t, y, how="gauss", isPlot=False):
...
def fit(X, how="gauss"):
    """fits from an interpolating function"""
...
def dis_Fermi(x, t, p):
    """Fermi distribution for a given chemical potential and thermal noise"""
    exp = np.exp((t - x[0]) / x[1])
    return 1. / (exp + 1.)


def fitFermi(y, isPlot=False, thermal_noise=0.75):
    """fits according to Fermi-Dirac distribution"""
    ...
    err = np.sqrt(sum((y - y1) ** 2)) / (sum(y1) + sum(y)) * 2
    if isPlot:
        plt.title("chem pot %.2f thermal noise %.2f" % (x0[0], x0[1]))
        plt.plot(t, y, label="empirical")
        plt.plot(t, y1, label="distribution")
        plt.legend()
        plt.show()
    return t, y1, x0, err
